...
- **초록**: OpenAlex 는 Elsevier(ScienceDirect) 논문의 초록을 안 준다(Crossref·Semantic Scholar 도 없음, 2026-09-24 확인). 정보 패널의 「찾기」·요약·번역은 초록이 없으면 `find_abstract` 로 보유 PDF 첫 2쪽(`a b s t r a c t` 띄어쓰기 포함) → Scopus(키가 있으면) → Crossref → Semantic Scholar 순으로 찾고. **Elsevier 무료 API 키**(환경설정 `elsevier_api_key`, `설정.json`)로 되는 것: Scopus 초록 API `/content/abstract/doi/` (모든 출판사 초록·저자 키워드·피인용, 주 10,000회), Scopus 검색 API(주 20,000회), ScienceDirect 검색 API. 안 되는 것: ScienceDirect 기사 API(META_ABS 도 403, 기관 IP 필요). 스마트 탐색은 Claude 선별 전에 초록 없는 후보를 `fill_abstracts_scopus` 로 채운다(4갈래 병렬, `관련맵\초록캐시.json` DOI 캐시). 찾은 초록은 `관련맵\탐색요약.json` 에 캐시(키 `id|abs`, 요약 `id`, 번역 `id|tr`). 검색 결과의 초록은 2500자까지 보관
